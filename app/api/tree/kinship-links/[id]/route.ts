...
import { deleteKinshipLink, updateKinshipLink } from "@/lib/tree-service";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!link) {
    return NextResponse.json({ error: "Kinship link not found" }, { status: 404 });
  }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const deleted = deleteKinshipLink({ id, treeId: parsed.data.treeId, actorUserId: user.id });
  if (!deleted) {
    return NextResponse.json({ error: "Kinship link not found" }, { status: 404 });
  }
